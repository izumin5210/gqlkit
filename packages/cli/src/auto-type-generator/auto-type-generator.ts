...
export interface AutoGeneratedType {
  ...
  readonly resolveTypeFieldPattern:
    | "typenameOnly"
    | "dollarTypenameOnly"
    | "mixed"
    | null;
...
function determineFieldPattern(
  memberTypenames: ReadonlyMap<number, MemberTypenameInfo>,
): AutoGeneratedType["resolveTypeFieldPattern"] {
  const fieldNames = new Set<MemberTypenameInfo["fieldName"]>();
  ...
  if (fieldNames.size === 1) {
    ...
    return fieldName === "__typename" ? "typenameOnly" : "dollarTypenameOnly";
  }
...
interface ExtractedTypenameInfo {
...
function extractTypenameFromInlineObject(
  ...
  if (
    selectedProperty.tsType.kind === "literal" &&
    selectedProperty.tsType.name !== null
  ) {
    return {
      typeName: selectedProperty.tsType.name,
      fieldName: typenameProperty ? "__typename" : "$typeName",
    };
  }

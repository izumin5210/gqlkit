...
function resolveNestedInlineObjectInMember(
  ...
  if (generatedTypeNames.has(contextKey)) {
    return generatedTypeNames.get(contextKey)!;
  }

  const fields: AutoGeneratedField[] = properties.flatMap((prop) => {
    ...
    if (
      ...
      const resolvedType = resolveNestedInlineObjectInMember({
        properties: prop.tsType.inlineObjectProperties,
        fieldName: prop.name,
      ...
      return {
        name: prop.name,
        type: { ...fieldType, typeName: resolvedType },
        description: prop.description,
        deprecated: prop.deprecated,
        directives: prop.directives,
        defaultValue: prop.defaultValue,
  ...
  generatedTypeNames.set(contextKey, typeName);
    ...
    if (memberType.kind === "reference" && memberType.name) {
      ...
      const fields: AutoGeneratedField[] = memberType.inlineObjectProperties
        ...
        .flatMap((prop) => {
          ...
          // Resolve nested inline objects within union member properties
          if (
            fieldType.typeName === "__INLINE_OBJECT__" &&
            prop.tsType.kind === "inlineObject" &&
            prop.tsType.inlineObjectProperties

  ...
  for (const autoType of autoGeneratedTypes) {
    ...
    if (userType) {
      diagnostics.push({
        ...
        message: `Auto-generated type '${autoType.name}' conflicts with user-defined type at ${formatLocation(userType.sourceLocation)}. Consider renaming the field or using an explicit type definition.`,
        severity: "error",
...
function detectAutoGeneratedCollisions(
  autoGeneratedTypes: ReadonlyArray<AutoGeneratedType>,
): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  ...
  for (const [name, types] of typesByName) {
    if (types.length > 1) {
      const locations = types
        .map((t) => formatLocation(t.sourceLocation))
        .join(", ");
...
export function validateNameCollisions(
  ...
  const userCollisions = detectUserDefinedCollisions(
    input.userDefinedTypes,
    input.autoGeneratedTypes,
  );

  const autoCollisions = detectAutoGeneratedCollisions(
    input.autoGeneratedTypes,
  );

...
function buildAutoResolveTypeEntry(unionTypeName: string): string {
...
function buildTypeEntries(
  ...
  const autoResolveTypeByUnion = new Set<string>(
    resolverInfo.autoGeneratedResolveTypes.map((r) => r.unionTypeName),
  );

  const allTypeNames = new Set<string>([
    ...typesWithFields.keys(),
    ...abstractResolversByType.keys(),
    ...autoResolveTypeByUnion,
  ]);
  ...
  for (const typeName of sortedTypeNames) {
    ...
    const hasAutoResolveType = autoResolveTypeByUnion.has(typeName);
    ...
    } else if (hasAutoResolveType) {
      typeEntries.push(buildAutoResolveTypeEntry(typeName));
    }
  }
